...
class UNet(nn.Module):
    def __init__(
        ...
        for i_level in range(self.num_resolutions):
            block = nn.ModuleList()
            attn = nn.ModuleList()
            block_in = model_channels * in_ch_mult[i_level]
            block_out = model_channels * ch_mult[i_level]
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            attn = nn.ModuleList()
            block_out = model_channels * ch_mult[i_level]
            skip_in = model_channels * ch_mult[i_level]
            for i_block in range(self.num_res_blocks + 1):
                if i_block == self.num_res_blocks:
                    skip_in = model_channels * in_ch_mult[i_level]
                block.append(
                    ResnetBlock(
    ...
    def forward(self, x):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                h = self.up[i_level].block[i_block](torch.cat([h, hs.pop()], dim=1))
